    ...
    def class_activation(self, test_loader):
        ...
        with torch.no_grad():
            for image, lbl in tqdm(test_loader):
                ...
                final_cam = F.interpolate(cam, (self.args.resize, int(self.resize*1.25)), mode="bilinear", align_corners=True)

                plt.figure()
                plt.imshow(np.asarray(image).squeeze().transpose(1,2,0))
                plt.imshow(final_cam.squeeze().detach().cpu().numpy(), alpha=0.3)
